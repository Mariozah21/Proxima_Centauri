from wtforms import Form, PasswordField, StringField, EmailField, IntegerField, DecimalField, RadioField, validators
class RegisterUserForm(Form):
    ...
    email = EmailField(name='email',label='Email',validators=[validators.length(min=2,max=100), validators.InputRequired(), validators.Email()])
...
class UserMoveForm(Form):
    email = EmailField(name='email',label='Email uzivatela',validators=[validators.length(min=2,max=100), validators.InputRequired(), validators.Email()])
...
class ChangeEmailForm(Form):
    novyemail = EmailField(name='novyemail',label='Novy Email',validators=[validators.length(min=2,max=100), validators.InputRequired(), validators.Email()])
    heslo = PasswordField(name='heslo',label='Heslo',validators=[validators.length(min=5,max=100), validators.InputRequired()])